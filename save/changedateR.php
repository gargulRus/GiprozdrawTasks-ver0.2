...
if ($del==2){
	...
	$result13 = query ("DELETE FROM periodR WHERE object_id='$obid'");
    $action="Снимаем дату";
    }else{
        $action="Даты нет!";
    }
...
foreach ($list as $key => $row) {
    ...
    if($row['plan'][5]['data']==NULL || $row['plan'][5]['data']==NULL){
    }else{
     if($row['plan'][5]['data_2']!=NULL){
         //бесполезная проверка!
        $datastart = $row['plan'][5]['data_2'];
    }else{
        $datastart = $row['plan'][5]['data'];
    }
    // проверяем есть ли data_2
     if($row['plan'][6]['data_2']!=NULL){
        $dataend = $row['plan'][6]['data_2'];
    }else{
        $dataend = $row['plan'][6]['data'];
    }

    //ищем разницу в днях между началом и концом
    $daydata = daydiff($datastart, $dataend);
    //Просчитываем дни для периодов
    $firstperiod = intval($daydata*0.4);
    $secondperiod = intval($daydata*0.4);
    $thirdperiod = intval($daydata*0.2);
...
$result11 = query("SELECT id, datastart, data_p1, data_p2, data_p3 FROM periodR WHERE object_id=".$obid);

$count = mysqli_num_rows($result11);

if( $count > 0 ) {
    //если есть запись обновляем
  $result10 = query ("UPDATE periodR SET datastart = '$datastart', data_p1 = '$makedate1', data_p2 = '$makedate2', data_p3 = '$makedate3' WHERE object_id='$obid'");
} else {
    //если нет - создаем
    $result12 = query ("INSERT INTO `periodR` (`object_id`, `datastart`, `data_p1`, `data_p2`, `data_p3`)
