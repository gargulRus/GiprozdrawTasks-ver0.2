...
<?php include(__DIR__.'/../template/plancontrolR-users.php'); 
            ...
            foreach ($pos_num as $key_m => $col) {
                if(isset($row['task'][$key_m]) && $row['task'][$key_m]['notuse']==1){
                    ...
                    if($row['task'][$key_m]['progress']<89){
                        ...
                        echo '<td><a 
                        href="/?modal=arhivR&pos_num='.$key_m.'&task_id='.$row['task'][$key_m]['id'].'&object_id='.$row['id'].'" class="modal-a openformtask">'. $row['task'][$key_m]['progress'] .'%
                        </a> </td>';
                    }
                }else{
                    echo '<td><a 
