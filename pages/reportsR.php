...
$list = array();
$result = query("SELECT id, object_id, pos_num, timeR, user  FROM plancontrolR WHERE user!='NULL' ORDER BY timeR DESC");
while($data = mysqli_fetch_assoc($result)){ 
    ...
    $toRazdelP = query("SELECT id, title FROM razdelR WHERE pos_num=".$data['pos_num']);
    $radzdelParr=array();
